...
- (RACSignal *)requestNetworkData:(FMHttpRequest *)req{
    ...
    RACSignal *signal = [RACSignal createSignal:^(id<RACSubscriber> subscriber) {
        ...
        NSLog(@"AAA = %@",[[NSURL URLWithString:req.path relativeToURL:[NSURL URLWithString:BaseUrl]] absoluteString]);
                ...
                if (statusCode == HTTPResponseCodeSuccess) {
                    ...
                    if (statusCode == HTTPResponseCodeNotLogin) {
                        ...
                        if ((msgTips.length == 0 ||
                            ...
                            msgTips = NSLocalizedString(@"NetWorkingStatus_04", nil);//服务器出错了，请稍后重试
                        }
    ...
    RACSignal *signal = [RACSignal createSignal:^(id<RACSubscriber> subscriber) {
        ...
        NSMutableURLRequest *request = [self.manager.requestSerializer multipartFormRequestWithMethod:@"POST"
                                                                                            URLString:[[NSURL URLWithString:path
                                                                                                              relativeToURL:[NSURL URLWithString:BaseUrl]]//上传的资源文件域名地址头
                                                                                                       absoluteString]
...
- (NSError *)errorFromRequestWithTask:(NSURLSessionTask *)task
    ...
    NSString *errorDesc = NSLocalizedString(@"NetWorkingStatus_04", nil);//服务器出错了，请稍后重试
    ...
    if (httpFirstCode > 0) {
        ...
        if (httpFirstCode == 4) {
            ...
            if (HTTPCode == 408) {
                ...
                errorDesc = NSLocalizedString(@"NetWorkingStatus_03", nil);//请求超时，请稍后再试
            }else{
                
                errorDesc = NSLocalizedString(@"NetWorkingStatus_02", nil);//请求出错了，请稍后重试
            }
        }else if (httpFirstCode == 5 || httpFirstCode == 6){
            /// 服务器出错了，请稍后重试
            errorDesc = NSLocalizedString(@"NetWorkingStatus_04", nil);//服务器出错了，请稍后重试
        }else if (!self.manager.reachabilityManager.isReachable){
            /// 网络不给力，请检查网络
            errorDesc = NSLocalizedString(@"NetWorkingStatus_01", nil);//网络开小差了，请稍后重试
        }
    }else{
    ...
    switch (HTTPCode) {
        ...
        default:
            ...
            if ([error.domain isEqual:NSURLErrorDomain]) {
                ...
                //                "NetWorkingStatus_01" = "网络开小差了，请稍后重试";
                //                "NetWorkingStatus_02" = "请求出错了，请稍后重试";
                //                "NetWorkingStatus_03" = "请求超时，请稍后再试";
                //                "NetWorkingStatus_04" = "服务器出错了，请稍后重试";
