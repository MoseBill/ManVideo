...
+ (instancetype)popoverAnimatorWithStyle:(YCPopoverType )popoverType
                          completeHandle:(YCCompleteHandle)completeHandle{
...
-(YCPresentationController *)presentationController{
    if (!_presentationController) {
        _presentationController = [[YCPresentationController alloc] initWithPresentedViewController:self.presented
                                                                           presentingViewController:self.presenting];

         _presentationController.popoverType = self.popoverType;
        if (self.popoverType == YCPopoverTypeAlert) {
            _presentationController.presentedSize = _presentedSize;
        }else{
            _presentationController.presentedHeight = self.presentedHeight;
        }
...
- (void)animationForDismissedView:(nonnull id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    if (self.popoverType == YCPopoverTypeAlert) {
        ...
        [UIView animateWithDuration:kAnimationDuration
                         animations:^{
            weakSelf.presentationController.coverView.alpha = 0.0f;
            presentedView.transform = CGAffineTransformMakeTranslation(0,
                                                                       weakSelf.presentedHeight);
        } completion:^(BOOL finished) {
            [presentedView removeFromSuperview];
            [transitionContext completeTransition:YES];
        }];
