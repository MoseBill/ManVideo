...
//                                                    NSFontAttributeName : [UIFont fontWithName:@"Helvetica-Bold" size:17]}];
    ...
    self.gk_navBackgroundColor = clear_color;
    //    self.gk_navTitleView = self.titleView;
    self.gk_statusBarStyle = UIStatusBarStyleLightContent;
    self.gk_navLineHidden = YES;
    self.gk_backStyle =  GKNavigationBarBackStyleWhite;
    self.view.backgroundColor = [UIColor colorWithHexString:@"050013"];
    UIBarButtonItem *rightitem=[[UIBarButtonItem alloc]initWithCustomView:self.frontBtn];
    UIBarButtonItem *rightItem2 = [[UIBarButtonItem alloc]initWithCustomView:self.flashBtn];
    NSArray *array = @[rightitem,rightItem2];
    self.gk_navRightBarButtonItems = array;
        //音频权限
...
- (void)flashSwitch:(UIButton *)btn
{
    ...
    if (btn.selected) {
        [btn setImage:[UIImage imageNamed:@"media_flash_0"] forState:UIControlStateNormal];
            //打开闪光灯
            NSLog(@"ON");
...
//            self.s.enabled = NO;
        ...
        [btn setImage:[UIImage imageNamed:@"media_flash_2"] forState:UIControlStateNormal];
        //关闭闪光灯
        NSLog(@"Off");
...
//    [btn setTitleColor:[UIColor colorWithRed:255.0f green:197.0f blue:2 alpha:1.0f] forState:UIControlStateNormal];
//
//        //设置
//    NSError *error = nil;
//    if ([captureDevice lockForConfiguration:&error]) {
...
//    }
//        //更新UI
//    UIImage *image = [UIImage imageNamed:@"media_flash_2"];
...
- (void)proportionAction:(UIButton *)sender {
    ...
    if (sender.selected) {
        [previewLayer setFrame:CGRectMake(0, 0,SCREEN_WIDTH,SCREEN_HEIGHT)];
        [self.proportionBtn setTitle:@"小屏" forState:UIControlStateNormal];
    }else{
        [previewLayer setFrame:CGRectMake(0, Height_NavBar,SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (UIButton *)frontBtn { //转换镜头按钮
    if (!_frontBtn) {
        _frontBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 20, 50, 20)];
//        [_frontBtn setBackgroundImage:[UIImage imageNamed:@"相机图标"] forState:UIControlStateNormal];
        [_frontBtn setImage:[UIImage imageNamed:@"相机图标"] forState:UIControlStateNormal];
...
- (UIButton *)flashBtn {//闪光灯
    if (!_flashBtn) {
        _flashBtn = [[UIButton alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-200)/2, 20, 40, 40)];
        [_flashBtn setImage:[UIImage imageNamed:@"media_flash_2"] forState:UIControlStateNormal];
        [_flashBtn addTarget:self action:@selector(flashSwitch:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _flashBtn;
}
...
//    if (!_flashView) {
//        _flashView = [[UIView alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-160)/2, 30, 150, 30)];
...
- (UIView *)toolView {//底部视图
    if (!_toolView) {
        _toolView = [[UIView alloc] initWithFrame: CGRectMake(0, SCREEN_HEIGHT - 120, SCREEN_WIDTH, 120)];
        _toolView.backgroundColor = kClearColor;
    }
    return _toolView;
}
...
- (UIButton *)photoBtn {//拍照开始按钮
    if (!_photoBtn) {
        _photoBtn = [[UIButton alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-80)/2,10, 80, 80)];
...
- (UIButton *)videoBtn {//录像开始按钮
    if (!_videoBtn) {
        _videoBtn = [[UIButton alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-80)/2,10, 80, 80)];
...
{
    if (!_switchBtn) {
        _switchBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH-100, 10, 50, 50)];
...
- (UIButton *) proportionBtn {//比例按钮
    if (!_proportionBtn) {
        _proportionBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH-60, 60, 50, 50)];
...
- (UIButton *)mapBtn {//添加贴纸按钮
    if (!_mapBtn ){
        _mapBtn = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH-150, 60, 50, 50)];
...
- (UIView *)mapView {//贴图视图
    if (!_mapView) {
        _mapView = [[UIView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT-120-150-1, SCREEN_WIDTH, 150)];
        _mapView.backgroundColor = [UIColor colorWithRed:87.0f green:87.0f blue:87.0 alpha:0.33];
    }
    return _mapView;
}
...
- (UIView *)filterView {//滤镜视图
    if (!_filterView) {
        _filterView = [[UIView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT-120-150-1, SCREEN_WIDTH, 150)];
        _filterView.backgroundColor = [UIColor colorWithRed:87.0f green:87.0f blue:87.0f alpha:0.33];
    }
    return _filterView;
}
...
- (UIImageView *)dotImageView { //录像开始后 时间点边上的闪烁点
    if (!_dotImageView) {
        ...
        _dotImageView.frame = CGRectMake((SCREEN_WIDTH-90)/2, 40, 5, 5);
...
- (UILabel *)timeLabel {//录像时间
    if (!_timeLabel) {
        _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-80)/2, 20, 80, 40)];
