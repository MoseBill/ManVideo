...
- (instancetype)initWithFrame:(CGRect)frame  searchBarStyle:(LGLSearchBarStyle)style {
    ...
    if (self) {
        ...
        _searchBar.placeholder = NSLocalizedString(@"VDsearchcontentText", nil);
...
- (void)searchBarTextDidBeginEditing:(UISearchBar *)searchBar{
    ...
    for(UIView *view in  [[[searchBar subviews] objectAtIndex:0] subviews]) {
        if([view isKindOfClass:[NSClassFromString(@"UINavigationButton") class]]) {
            ...
            [cancel setTitle:NSLocalizedString(@"VDSearchText", nil) forState:UIControlStateNormal];
            [cancel setTitleColor:[UIColor colorWithHexString:@"EB3053"] forState:UIControlStateNormal];
