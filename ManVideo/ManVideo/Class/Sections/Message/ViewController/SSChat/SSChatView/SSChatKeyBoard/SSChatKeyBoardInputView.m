...
//        [self addSubview:_topLine];
        ...
        [_mLeftBtn setBackgroundImage:[UIImage imageNamed:@"icon_yuying-1"] forState:UIControlStateNormal];
        ...
        [_mAddBtn setBackgroundImage:[UIImage imageNamed:@"sum"] forState:UIControlStateNormal];
        [_mAddBtn setBackgroundImage:[UIImage imageNamed:@"sum"] forState:UIControlStateSelected];
        _mAddBtn.selected = NO;
        ...
        [_mSymbolBtn setBackgroundImage:[UIImage imageNamed:@"斯米兰"] forState:UIControlStateNormal];
...
//        [_mTextBtn setTitle:@"松开 结束" forState:UIControlStateHighlighted];
        ...
        _mTextView.backgroundColor = [UIColor colorWithHexString:@"262233"];
        _mTextView.returnKeyType = UIReturnKeySend;
        _mTextView.font = [UIFont systemFontOfSize:15];
        _mTextView.textColor = kWhiteColor;
        ...
        _mKeyBordView.userInteractionEnabled = YES;
...
- (void)SetSSChatKeyBoardInputViewEndEditing {
    ...
    if(self.keyBoardStatus != SSChatKeyBoardStatusVoice){
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"sumNotification" object:@"1"];
        self.mAddBtn.userInteractionEnabled = YES;
    }
}
//语音10  表情11  其他功能12
...
//    NSLog(@"点击加号%ld",sender.tag);
    sender.selected = !sender.selected;
    if (sender.selected) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"sumNotification" object:@"0"];
        self.keyBoardHieght = SafeAreaBottom_Height+SSChatKeyBordHeight;
//        self..userInteractionEnabled = NO;
        self.mAddBtn.userInteractionEnabled = NO;
    } else {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"sumNotification" object:@"1"];
        self.keyBoardHieght = 0.0;
          self.mAddBtn.userInteractionEnabled = YES;
...
- (void)beginRecordVoice:(UIButton *)button{
    ...
    if (error || !_recorder) {
        DLog(@"recorder: %@ %zd %@", [error domain], [error code], [[error userInfo] description]);
        return;
    }
