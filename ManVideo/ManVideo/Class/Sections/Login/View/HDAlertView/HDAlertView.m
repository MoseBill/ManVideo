...
#define HDiPhone6ScaleWidth(width) (HDiPhone6WidthCoefficient(width) * SCREEN_WIDTH)     // 以苹果6为准的系数得到的宽
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    UIView *view = self.subviews[idx];
                    UIVisualEffectView *effectView = view.subviews[0];
                    effectView.backgroundColor =kClearColor;
                    UIView *subView = view.subviews[2];
                    subView.backgroundColor = [UIColor colorWithHexString:@"272333" alpha:0.8];
                    UIView *viewEff = effectView.subviews[0];
                    viewEff.backgroundColor = kClearColor;
        }];
    }
...
- (void)show {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:HDAlertViewWillShowNotification
                                                        object:self
                                                      userInfo:nil];
    self.visible = YES;
    ...
    [self transitionInCompletion:^{
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:HDAlertViewDidShowNotification
                                                            object:self
                                                          userInfo:nil];
        self.alertAnimating = NO;
    }];
...
- (void)dismissAnimated:(BOOL)animated cleanup:(BOOL)cleanup {
    ...
    if (self.isVisible) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:HDAlertViewWillDismissNotification
                                                            object:self
                                                          userInfo:nil];
    }
    ...
    void (^dismissComplete)(void) = ^{
        ...
        if (isVisible) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:HDAlertViewDidDismissNotification
                                                                object:self
                                                              userInfo:nil];
        }
        [self removeView];
...
- (void)updateSubviewsFrame {
    if (self.alertViewStyle == HDAlertViewStyleActionSheet) {
        CGFloat containerViewW = SCREEN_WIDTH;
        CGFloat margin = 25.0;
...
//                    lineY = buttonY;
        ...
        CGFloat containerViewW = (SCREEN_WIDTH - horizontalMargin * 2);
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index {
    ...
    [button setBackgroundImage:[UIImage hd_imageWithColor:[RGBCOLOR(230, 230, 230) colorWithAlphaComponent:0.5]] forState:UIControlStateHighlighted];
...
#pragma mark - 类方法
+ (HDAlertView *)showAlertViewWithTitle:(NSString *)title
                                message:(NSString *)message
                      cancelButtonTitle:(NSString *)cancelButtonTitle
                      otherButtonTitles:(NSArray<NSString *> *)otherButtonTitles
                                handler:(void (^)(HDAlertView *alertView,
                                                  NSInteger buttonIndex))block {
        HDAlertView *alertView = [[HDAlertView alloc] initWithTitle:title
                                                         andMessage:message];
        alertView.cancelButtonTitleColor = [UIColor blackColor];
        alertView.defaultButtonTitleColor = UIColorFromRGB(0x0093ff);
        alertView.destructiveButtonTitleColor = UIColorFromRGB(0xf74c31);
        alertView.buttonFont = [UIFont systemFontOfSize:17.0];
        alertView.isSupportRotating = YES;
        if (![cancelButtonTitle isNullString]) {
...
+ (HDAlertView *)showActionSheetWithTitle:(NSString *)title {
    ...
    alertView.buttonFont = [UIFont systemFontOfSize:17.0];
    alertView.cancelButtonTitleColor = UIColorFromRGB(0xf74c31);
    alertView.defaultButtonTitleColor = [UIColor blackColor];
    alertView.destructiveButtonTitleColor = UIColorFromRGB(0xf74c31);
    alertView.titleColor = UIColorFromRGB(0x999999);
