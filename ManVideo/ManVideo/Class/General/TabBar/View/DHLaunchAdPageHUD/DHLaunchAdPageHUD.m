...
- (UIImage *)launchImage {
    ...
    DLog(@"[DHLaunchAdPageHUD]:请添加启动图片");
    return nil;
}
...
- (UIButton *)setUpSkipButton {
    if (self.skipButton == nil) {
        ...
        [self.skipButton setTitle:[NSString stringWithFormat:@"%ld", duration] forState:UIControlStateNormal];
...
//        @strongify(self)
        if (!weaktarget) {
            dispatch_source_cancel(weaktarget.timer);
        } else {
        dispatch_async(dispatch_get_main_queue(), ^{
            duration--;
            if (duration > -1) {
                [weaktarget.skipButton setTitle:[NSString stringWithFormat:@"%ld", duration] forState:UIControlStateNormal];
            }
        });
             }
    });
...
//        @weakify(self)
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
//            @strongify(self)
            [[[UIApplication sharedApplication].delegate window] addSubview:weakSelf];
        });
    }];
}
