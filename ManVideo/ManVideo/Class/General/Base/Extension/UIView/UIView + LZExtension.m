...
-(BOOL)isShowingOnWindow{
    //转换坐标系
    CGRect newFrame = [self.superview convertRect:self.frame
                                           toView:kAPPWindow];
    CGRect windowBouns = kAPPWindow.bounds;
    BOOL intersects =  CGRectIntersectsRect(newFrame, windowBouns);
    
    //判断一个控件是否真正显示在窗口范围内
    return !self.isHidden && self.alpha > 0.01 && intersects && self.window == kAPPWindow;
}
